...
with tf.Graph().as_default():
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options,
        ...
        image, ih, iw, gt_boxes, gt_masks, num_instances, img_id = \
            coco.read('./data/coco/records/coco_train2014_00000-of-00033.tfrecord')
        ...
        outputs = pyramid_network.build_heads(pyramid, ih, iw, num_classes=81, base_anchors=9, is_training=True,
                                              gt_boxes=gt_boxes)
        ...
        # print(" ---- FUCK1 ----")
        # # tmpImage = sess.run(gt_boxes)
        # tmpImage = image.eval()
        # tmpMask = gt_masks.eval()
        # print('Image shape = {0}'.format(tmpImage.shape))
        # for ii in range(3):
        #     plt.subplot(1, 4, ii + 1),
        #     plt.imshow(tmpImage[0][:, :, ii])
        #     plt.title('shape={0} : {1}'.format(tmpImage.shape, ii))
        # plt.subplot(1, 4, 4)
        # plt.imshow(np.argmax(tmpMask, axis=0))
        # plt.show()
        # print(" ---- FUCK2 ----")
        # sys.exit(0)
