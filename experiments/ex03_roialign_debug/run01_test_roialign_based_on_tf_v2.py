...
class RoiAligngConv_V1(Layer):
    '''ROI Align layer for 2D inputs.
    Implementation base on tf.image.crop_and_resize(...)
    # Arguments
        pool_size: int
...
if __name__ == '__main__':
    ...
    dataROI = np.array([(xx/float(nx), yy/float(ny), 1./nx, 1./ny) for yy in range(ny) for xx in range(nx)], dtype=np.float32)
    dataROI = np.expand_dims(dataROI, axis=0)
    ...
    numch = tret.shape[-1]
    for nn in range(numch):
        cnt = 0
        for yy in range(ny):
            for xx in range(nx):
                plt.subplot(ny, nx, cnt+1)
                plt.imshow(tret[0][cnt][:,:,nn])
                plt.title('(x,y) = ({0},{1}), #ch={2}/{3}'.format(xx,yy, nn+1, numch))
                cnt+=1
        plt.show()
    print ('-')