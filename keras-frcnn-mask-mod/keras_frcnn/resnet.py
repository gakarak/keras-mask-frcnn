...
def nn_base(input_tensor=None, trainable=False, isDebug=False):
    ...
    if isDebug:
        ...
        tmpModel = keras.models.Model(inputs=img_input, outputs=x)
        tmpImage = "tmp_debug_image.png"
        keras.utils.plot_model(tmpModel, tmpImage, show_shapes=True)
        plt.imshow(skio.imread(tmpImage))
        plt.show()
    return x
